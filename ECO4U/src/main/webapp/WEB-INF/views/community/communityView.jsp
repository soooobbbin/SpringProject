	...
	<div class="align-right">
		...
		<c:if test="${user.auth == 2 && community.c_auth==0}">
			...
			notice_btn.onclick=function(){
				...
				if(choice){
					location.replace('updatenotice.do?c_num=${community.c_num}');
				}
			};
		</script>
		</c:if>
		<c:if test="${user.auth == 2 && community.c_auth==1}">
			...
			notice_btn.onclick=function(){
				...
				if(choice){
					location.replace('updatenotice2.do?c_num=${community.c_num}');
				}
			};
		</script>
		</c:if>
	...
	<div id="comment_div">
		...
		<form id="com_form">
			<input type="hidden" name="c_num" value="${community.c_num}"
				id="c_num">
			<textarea rows="5" cols="65" name="com_content" id="com_content"
				class="comm-content"
